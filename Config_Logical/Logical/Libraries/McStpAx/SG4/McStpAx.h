...
#define _McStpAx_VERSION 5.26.1
#endif
...
typedef enum McStpAxMotorCurrentEnum
{	mcSTPAX_CURRENT_AUTOMATIC = 0,
	mcSTPAX_CURRENT_STANDSTILL,
	mcSTPAX_CURRENT_CONST_SPEED,
	mcSTPAX_CURRENT_SPEED_CHANGE
} McStpAxMotorCurrentEnum;

typedef enum McStpAxUsedMotorCurrentEnum
{	mcSTPAX_USED_CURRENT_OFF = 0,
	mcSTPAX_USED_CURRENT_STANDSTILL,
	mcSTPAX_USED_CURRENT_CONST_SPEED,
	mcSTPAX_USED_CURRENT_SPEED_CHG
} McStpAxUsedMotorCurrentEnum;

typedef enum McSAMRotDirEnum
{	mcSAMRD_STD = 0,
	mcSAMRD_INV = 1
} McSAMRotDirEnum;
...
{	mcSAELOEERP_NOT_USE = 0,
	mcSAELOEERP_IO_CH = 1,
	mcSAELOEERP_VAR = 2,
	mcSAELOEERP_IO_CH_DINT = 3,
	mcSAELOEERP_VAR_DINT = 4
} McSAELOEExtRefPEnum;
...
typedef enum McSACMPACCtrlPosAutAdjEnum
{	mcSACMPACCPAA_NOT_USE = 0,
	mcSACMPACCPAA_VELBASED = 1
} McSACMPACCtrlPosAutAdjEnum;
...
typedef enum McSAHModClrPosErrEnum
{	mcSAHMCPE_NO = 0,
	mcSAHMCPE_YES = 1
} McSAHModClrPosErrEnum;
...
typedef enum McSSLdSimModEnum
{	mcSSLSM_NOT_USE = 0,
	mcSSLSM_SET_VAL_GEN = 1
} McSSLdSimModEnum;
...
typedef enum McSMActModSimOnPLCEnum
{	mcSMAMSOP_OFF = 0,
	mcSMAMSOP_ON = 1
} McSMActModSimOnPLCEnum;
...
{	enum McHomingModeEnum HomingMode;
	...
	enum McSwitchEnum ClearPositionError;
...
{	float SteppingAngle;
	enum McSAMRotDirEnum RotationDirection;
} McSAMType;
...
typedef struct McSAELOEExtRefPIOChDINTPosType
{	plcstring ChannelMapping[251];
} McSAELOEExtRefPIOChDINTPosType;

typedef struct McSAELOEExtRefPIOChDINTCntType
{	plcstring ChannelMapping[251];
} McSAELOEExtRefPIOChDINTCntType;

typedef struct McSAELOEExtRefPIOChDINTType
{	struct McSAELOEExtRefPIOChDINTPosType Position;
	struct McSAELOEExtRefPIOChDINTCntType Count;
} McSAELOEExtRefPIOChDINTType;

typedef struct McSAELOEExtRefPVarDINTPosType
{	plcstring PVMapping[251];
} McSAELOEExtRefPVarDINTPosType;

typedef struct McSAELOEExtRefPVarDINTCntType
{	plcstring PVMapping[251];
} McSAELOEExtRefPVarDINTCntType;

typedef struct McSAELOEExtRefPVarDINTType
{	struct McSAELOEExtRefPVarDINTPosType Position;
	struct McSAELOEExtRefPVarDINTCntType Count;
} McSAELOEExtRefPVarDINTType;
...
{	enum McSAELOEExtRefPEnum Type;
	...
	struct McSAELOEExtRefPIOChDINTType IOChannelDINT;
	struct McSAELOEExtRefPVarDINTType VariableDINT;
} McSAELOEExtRefPType;
...
typedef struct McSACMPACCtrlPosAutAdjVelBsdType
{	float Level;
	float Zone;
} McSACMPACCtrlPosAutAdjVelBsdType;

typedef struct McSACMPACCtrlPosAutAdjType
{	enum McSACMPACCtrlPosAutAdjEnum Type;
	struct McSACMPACCtrlPosAutAdjVelBsdType VelocityBased;
} McSACMPACCtrlPosAutAdjType;
...
{	float ProportionalGain;
	...
	struct McSACMPACCtrlPosAutAdjType AutomaticAdjustment;
...
{	struct McSAHModType Mode;
	...
	enum McSAHModClrPosErrEnum ClearPositionError;
} McSAHType;
...
typedef struct McSAZVFUseType
{	float MaximumZeroVibrationFilterTime;
	float ZeroVibrationFilterCoefficient;
	float ZeroVibrationFilterTime;
} McSAZVFUseType;

typedef struct McSAZVFType
{	enum McSAZVFEnum Type;
	struct McSAZVFUseType Used;
} McSAZVFType;
...
typedef struct McSSLdSimModType
{	enum McSSLdSimModEnum Type;
} McSSLdSimModType;

typedef struct McSSType
{	struct McSSLdSimModType LoadSimulationMode;
} McSSType;
...
{	struct McCfgReferenceType AxisReference;
	...
	struct McSAZVFType ZeroVibrationFilter;
	struct McSADIType DigitalInputs;
	struct McSADOType DigitalOutputs;
	struct McSSType Simulation;
...
typedef struct McCfgStpAxZeroVibFltrType
{	struct McSAZVFType ZeroVibrationFilter;
} McCfgStpAxZeroVibFltrType;
...
typedef struct McCfgStpAxSimType
{	struct McSSType Simulation;
} McCfgStpAxSimType;
...
typedef struct McCfgStpModType
{	enum McSMActModSimOnPLCEnum ActivateModuleSimulationOnPLC;
} McCfgStpModType;
...
typedef struct MC_BR_SelectMotorCurrent_StpAx
{
	/* VAR_INPUT (analog) */
	struct McAxisType* Axis;
	enum McStpAxMotorCurrentEnum MotorCurrent;
	/* VAR_OUTPUT (analog) */
	signed long ErrorID;
	enum McStpAxUsedMotorCurrentEnum UsedMotorCurrent;
	/* VAR (analog) */
	struct McInternalType Internal;
	/* VAR_INPUT (digital) */
	plcbit Enable;
	plcbit SetMotorCurrent;
	/* VAR_OUTPUT (digital) */
	plcbit Valid;
	plcbit Busy;
	plcbit Error;
} MC_BR_SelectMotorCurrent_StpAx_typ;



/* Prototyping of functions and function blocks */
_BUR_PUBLIC void MC_BR_InitHome_StpAx(struct MC_BR_InitHome_StpAx* inst);
_BUR_PUBLIC void MC_BR_SelectMotorCurrent_StpAx(struct MC_BR_SelectMotorCurrent_StpAx* inst);
